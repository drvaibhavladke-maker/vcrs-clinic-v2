...
/* Neurodivergent Reset Plan — default template text per phase.
   Clinician edits/trims this per patient before saving, same pattern as PRESCRIPTION_PROTOCOL_DEFAULT. */
const NDRP_PHASE1_DEFAULT = `Goals: Identify strengths and challenges · Reduce overwhelm · Establish baseline

Assessment:
- Executive function screening
- ADHD/Autism screening (if indicated)
- Anxiety and depression assessment
- Sleep assessment
- Lifestyle evaluation
- Occupational functioning
- Sensory profile`;
const NDRP_PHASE2_DEFAULT = `Morning Reset (15-20 min):
...
Work Block:
- 45 minutes focus -> 10 minutes break, repeat 3-4 cycles
- During breaks: walk, hydrate, stretch, deep breathing
...
const NDRP_PHASE3_DEFAULT = `Lifestyle:
- Sleep 7-9 hours
- Hydration 2-3 L/day
...
- Visual planner / digital calendar / reminder systems

Physical Exercise:
- 150 minutes/week (walking, cycling, swimming, yoga, resistance training)`;
const NDRP_PHASE4_DEFAULT = `Daily Breathing: 4-6 breathing or Box Breathing, 5-10 minutes
Mindfulness: 10-15 minutes/day (guided meditation, progressive muscle relaxation, body scan)
Sensory Regulation: quiet room, weighted blanket, soft lighting, fidget tools, noise reduction`;
const NDRP_PHASE5_DEFAULT = `Weekly Success Journal — record:
...
- Replace "I never finish anything" with "I completed one important task today."
- Replace "I'm failing" with "I'm making progress at my own pace."`;
const NDRP_PHASE6_DEFAULT = `Tools: Google Calendar, Notion, Microsoft To Do, Trello, sticky notes, visual timers
Break every large task into: Goal -> Subtask -> 5-minute action`;
const NDRP_PHASE7_DEFAULT = `- Scheduled social interaction
- Family communication
- Support group (if helpful)
- Occupational therapy or coaching (when indicated)
- Psychotherapy (CBT/ACT) if anxiety or depression coexist`;
const NDRP_NUTRITION_DEFAULT = `Discuss with a healthcare professional before starting supplements. Possible supportive nutrients (only if appropriate): Omega-3 fatty acids, Vitamin D (if deficient), Vitamin B12 (if deficient), Iron (if deficient), Magnesium (selected cases).`;
const NDRP_MEDICATION_DEFAULT = `If symptoms significantly impair work, education, or daily life, evaluation by a psychiatrist or neurologist may be appropriate. Depending on diagnosis, treatment could include: stimulant medications (ADHD), non-stimulant ADHD medications, SSRIs or other medications for co-existing anxiety or depression, sleep-focused interventions when insomnia is present. Medication decisions should be individualized and supervised by a qualified clinician.`;
const NDRP_WEEKLY_GOALS_DEFAULT = `Week 1: Establish sleep schedule and routine
Week 2: Begin structured work blocks
Week 3: Add regular exercise and mindfulness
Week 4: Organize workspace and digital systems
Week 5: Build confidence through small daily wins
Week 6: Review progress and adjust strategies`;
const NDRP_EXPECTED_OUTCOMES = "Expected outcomes (6-12 weeks): improved daily structure and routine, better concentration and reduced mental fog, lower stress and anxiety, improved emotional regulation, enhanced executive functioning, increased confidence and self-efficacy, better work/academic productivity, improved quality of life.";
...
const MODULES = [
  ...
  },
  {
    ...
    fields: [
      { name: "Patient ID", db: "patient_id", type: "fk", module: "patients", required: true },
      { name: "Plan Start Date", db: "plan_date", type: "date", required: true },
      { name: "Phase 1 — Assessment", db: "phase1_assessment", type: "textarea", rows: 6, default: NDRP_PHASE1_DEFAULT },
      { name: "Phase 2 — Daily Structure", db: "phase2_daily_structure", type: "textarea", rows: 8, default: NDRP_PHASE2_DEFAULT },
      { name: "Phase 3 — Reduce Mental Fog", db: "phase3_mental_fog", type: "textarea", rows: 8, default: NDRP_PHASE3_DEFAULT },
      { name: "Phase 4 — Stress Regulation", db: "phase4_stress_regulation", type: "textarea", rows: 4, default: NDRP_PHASE4_DEFAULT },
      { name: "Phase 5 — Confidence Building", db: "phase5_confidence", type: "textarea", rows: 6, default: NDRP_PHASE5_DEFAULT },
      { name: "Phase 6 — Executive Function Support", db: "phase6_executive_function", type: "textarea", rows: 3, default: NDRP_PHASE6_DEFAULT },
      { name: "Phase 7 — Social & Emotional Well-being", db: "phase7_social_emotional", type: "textarea", rows: 5, default: NDRP_PHASE7_DEFAULT },
      { name: "Nutritional Support", db: "nutritional_support", type: "textarea", rows: 3, default: NDRP_NUTRITION_DEFAULT },
      { name: "Medication Considerations", db: "medication_considerations", type: "textarea", rows: 4, default: NDRP_MEDICATION_DEFAULT },
      { name: "Weekly Goals (6 Weeks)", db: "weekly_goals", type: "textarea", rows: 6, default: NDRP_WEEKLY_GOALS_DEFAULT },
      { name: "Clinician Notes", db: "clinician_notes", type: "textarea", rows: 3 },
      { name: "Status", db: "status", type: "select", options: ["Active", "Completed", "Discontinued"] },
    ],
    listColumns: ["Patient ID", "Plan Start Date", "Status"],
  },
  {
...
function PrintDocument({ type, record, patient, data, hideWrapperId }) {
  ...
  return (
    <div id={hideWrapperId ? undefined : "printable-area"} style={{ fontFamily: "Inter, sans-serif", color: "#16302B", padding: "24px", maxWidth: "700px", margin: "0 auto" }}>
      ...
      ) : type === "neurodivergentplan" ? (
        <>
          <h2 style={{ fontFamily: "Fraunces, serif", fontSize: "17px", borderBottom: "1px solid #DCE3DD", paddingBottom: "6px", textAlign: "center" }}>Neurodivergent Reset Plan</h2>
          <p style={{ margin: "10px 0 14px", fontSize: "10.5px", fontStyle: "italic" }}>A behavioral and lifestyle intervention framework — not a replacement for professional diagnosis or treatment.</p>
          <div style={{ fontSize: "12px", lineHeight: 1.5 }}>
            {[
              ["Phase 1 — Assessment", record.phase1_assessment],
              ["Phase 2 — Daily Structure", record.phase2_daily_structure],
              ["Phase 3 — Reduce Mental Fog", record.phase3_mental_fog],
              ["Phase 4 — Stress Regulation", record.phase4_stress_regulation],
              ["Phase 5 — Confidence Building", record.phase5_confidence],
              ["Phase 6 — Executive Function Support", record.phase6_executive_function],
              ["Phase 7 — Social & Emotional Well-being", record.phase7_social_emotional],
              ["Weekly Goals (6 Weeks)", record.weekly_goals],
              ["Nutritional Support", record.nutritional_support],
              ["Medication Considerations", record.medication_considerations],
          ...
          <p style={{ margin: "14px 0 0", fontSize: "10.5px", fontStyle: "italic" }}>{NDRP_EXPECTED_OUTCOMES}</p>
          {record.status && <p style={{ margin: "10px 0 0", fontSize: "12px" }}><strong>Status:</strong> {record.status}</p>}
        </>
      ) : (
        <>
